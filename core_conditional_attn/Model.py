...
class AssembleModel(nn.Module):
    def __init__(self, n_features, n_rules, d_hid, n_class):
        ...
        self.attention = AttentionModel(n_features + n_rules, n_rules, d_hid, n_class)
    ...
    def forward(self, x_l, x_u, x_lf_l, x_lf_u):
        ...
        fix_score = F.softmax(torch.mean(all_scores, dim=0), dim=0)  # use the average as the source reliability source A
